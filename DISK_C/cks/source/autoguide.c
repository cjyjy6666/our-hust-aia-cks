...
void draw_autoguide()
{
	...
	bar1(64,295,173,359, 0x7BEF);//1鏍?
    bar2(64,295,173,359,0x000000);
	bar1(65,631,189,742, 0x7BEF);//2鏍?
    bar2(65,631,189,742,0x000000);
	bar1(223,506,344,599, 0x7BEF);//3鏍? 
    bar2(223,506,344,599,0x000000);
	bar1(436,330,612,426, 0x7BEF);//4鏍?
    bar2(436,330,612,426,0x000000);
	bar1(450,630,605,727, 0x7BEF);//5鏍?
	bar2(450,630,605,727,0x000000);
    bar1(819,316,959,427, 0x7BEF);//6鏍?
...
void InitializeTable(int start,TABLE T[])//鍒濆?嬪寲琛ㄦ牸	
{
	int i;
	for(i=0;i<Max;i++)
	{
...
QUEUE InitializeQueue(int capacity)//鍒濆?嬪寲闃熷垪
{
    int i;
	QUEUE Q;
